...
export const CargoMonitoringPage: React.FC<CargoMonitoringPageProps> = ({ telemetry, trucks, lang = 'en' }) => {
  const t = translations[lang] || translations['en'];

  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700 }}>{t.cargoMonitoringTitle}</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          {t.cargoMonitoringSubtitle}
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        {trucks.map(truck => {
          const latest = telemetry.find(tr => tr.truckId === truck.truckId) || telemetry[0];
            ...
            <div key={truck.truckId} className="glass-panel" style={{ padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px' }}>
                ...
                <span className={`badge ${isOverload ? 'badge-danger' : 'badge-safe'}`}>
                  {isOverload ? t.overloadDetected : t.safeCapacity}
                </span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div style={{ background: gasSpike ? 'rgba(239, 68, 68, 0.1)' : 'rgba(255,255,255,0.03)', border: gasSpike ? '1px solid rgba(239, 68, 68, 0.3)' : '1px solid var(--border-color)', padding: '12px', borderRadius: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{t.mq135GasSensor}</span>
                  </div>
                  ...
                  <div style={{ fontSize: '0.65rem', color: gasSpike ? '#f87171' : 'var(--text-muted)', marginTop: '4px' }}>
                    {gasSpike ? t.abnormalGasDesc : t.normalAirQuality}
                  </div>
                </div>
                ...
                <div style={{ background: rainDetected ? 'rgba(245, 158, 11, 0.1)' : 'rgba(255,255,255,0.03)', border: rainDetected ? '1px solid rgba(245, 158, 11, 0.3)' : '1px solid var(--border-color)', padding: '12px', borderRadius: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{t.rainWaterIngress}</span>
                  </div>
                  <div style={{ fontSize: '1.1rem', fontWeight: 700, color: rainDetected ? '#fbbf24' : '#34d399' }}>
                    {rainDetected ? t.rainDetected : t.drySeal}
                  </div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                    {rainDetected ? t.waterIngressWarning : t.tarpaulinSecure}
                  </div>
                </div>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', padding: '12px', borderRadius: '8px', gridColumn: 'span 2' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 600 }}>{t.humiditySensor}</span>
                  </div>
